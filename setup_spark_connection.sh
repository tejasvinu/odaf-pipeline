...
docker exec -it $(docker ps -qf "name=airflow-webserver") airflow connections delete spark_default || true
docker exec -it $(docker ps -qf "name=airflow-webserver") airflow connections add spark_default \
    --conn-type spark \
    --conn-host local \
    --conn-port "" \
